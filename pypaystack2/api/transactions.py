...
class Transaction(BaseAPI):
    ...
    def check_authorization(
        ...
        Args:
            amount: Amount should be in kobo if currency is ``Currency.NGN``, pesewas, if currency is
                ``Currency.GHS``, and cents, if currency is ``Currency.ZAR``
            email: Customer's email address
            auth_code: Valid authorization code to charge
            currency: Currency in which amount should be charged. Any value from the ``Currency`` enum.
        ...
        url = self._parse_url("/transaction/check_authorization")
        payload = {
            "authorization_code": auth_code,
            "email": email,
            "amount": amount,
    ...
    def get_transfer_banks(self) -> Response:
...
class AsyncTransaction(BaseAsyncAPI):
    ...
    async def get_transfer_banks(self) -> Response:
        # TODO: Deprecate. it's available in Miscellaneous API
        """Fetch transfer banks
    ...
    async def create_transfer_customer(
        self, bank_code: str, account_number: int, account_name: str
    ) -> Response:
        # TODO: Deprecate. it's available in TransferReceipt API
        """Create a transfer customer
